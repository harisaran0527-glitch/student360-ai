        ...
        <div className="ui-card p-6 space-y-6">
          <Tabs
            tabs={[
              ...
              { id: "opportunities", label: "Campus Internship Opportunities", count: opportunities.length, icon: Calendar },
            ]}
            activeTab={activeTab}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
              {companies.map((c) => (
                <div key={c.name} className="p-4 rounded-xl border border-slate-200 dark:border-slate-800 space-y-2 bg-slate-50/50">
                  ...
                  <Badge variant="purple">{c.verifiedCount} Verified Internships</Badge>
                </div>
              ))}
            </div>
          )}
