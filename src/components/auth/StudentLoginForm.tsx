...
import { GraduationCap, ArrowRight, Cpu, Layers } from "lucide-react";
...
export function StudentLoginForm() {
  ...
  useEffect(() => {
    ...
    // Background floating particles (purple Tint)
    ...
    const render = () => {
      ...
      if (anim.particles) {
        ...
        ctx.fillStyle = `rgba(168, 85, 247, ${0.15 * particlesOpacity})`; // Purple Tint
        particles.forEach((p) => {
          p.y -= p.speed;
      ...
      if (anim.brainStart) {
        ...
        projected.forEach((node) => {
          ...
          let color = "rgba(168, 85, 247, 0.75)"; // Purple (left lobe)
          if (node.lobe === 'R') color = "rgba(6, 182, 212, 0.75)"; // Cyan (right lobe)
  ...
  return (
    <main className="min-h-screen bg-[#030611] flex flex-col justify-center items-center p-4 relative overflow-hidden font-sans select-none">
      ...
      <div 
        className="absolute top-[-10%] left-[-10%] w-[55%] h-[55%] bg-purple-950/10 rounded-full blur-[140px] pointer-events-none ambient-glow-1 transition-transform duration-[600ms] ease-out" 
        style={{ transform: `translate(${mousePos.x * 0.4}px, ${mousePos.y * 0.4}px)` }}
      />
      <div 
        className="absolute bottom-[-10%] right-[-10%] w-[55%] h-[55%] bg-cyan-950/10 rounded-full blur-[140px] pointer-events-none ambient-glow-2 transition-transform duration-[600ms] ease-out" 
        style={{ transform: `translate(${mousePos.x * -0.4}px, ${mousePos.y * -0.4}px)` }}
      />
      ...
      <div className="w-full max-w-6xl flex flex-col lg:flex-row items-center justify-between gap-8 z-10">
        ...
        <div className={`hidden lg:flex flex-col w-[260px] glass-futuristic p-5 rounded-2xl border border-purple-500/15 shadow-lg shadow-purple-950/20 transition-all duration-[1500ms] ease-out ${
          ...
          <div className="flex items-center gap-2 mb-3 text-purple-400">
            ...
            <span className="text-[11px] font-bold tracking-widest text-neon-purple uppercase">Cognitive Lifecycle</span>
          </div>
          <div className="space-y-3 font-mono text-[10px] text-slate-400">
            <div className="p-2 bg-slate-950/50 rounded-lg border border-slate-800/80">
              <span className="text-slate-500">PORTAL CONTEXT:</span>
              <span className="text-purple-400 ml-1.5 font-bold">STUDENT PORTFOLIO</span>
            </div>
            <div className="p-2 bg-slate-950/50 rounded-lg border border-slate-800/80">
              <span className="text-slate-500">LIFECYCLE STATUS:</span>
              <span className="text-emerald-400 ml-1.5 font-semibold">SYNCHRONIZED</span>
            </div>
            <div className="p-2 bg-slate-950/50 rounded-lg border border-slate-800/80">
              <span className="text-slate-500">PREDICTIVE FIT:</span>
              <span className="text-cyan-400 ml-1.5">99.4% confidence</span>
            </div>
            <div className="p-2 bg-slate-950/50 rounded-lg border border-slate-800/80 flex flex-col gap-1">
              <span className="text-slate-500">CONTEXT LIST:</span>
              <div className="flex flex-wrap gap-1.5 mt-1 text-[8px] font-semibold">
                <span className="bg-slate-900 border border-slate-800 px-1 py-0.5 rounded text-purple-300">academics</span>
                <span className="bg-slate-900 border border-slate-800 px-1 py-0.5 rounded text-purple-300">attendance</span>
                <span className="bg-slate-900 border border-slate-800 px-1 py-0.5 rounded text-purple-300">projects</span>
                <span className="bg-slate-900 border border-slate-800 px-1 py-0.5 rounded text-purple-300">placements</span>
                <span className="bg-slate-900 border border-slate-800 px-1 py-0.5 rounded text-purple-300">skills</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="relative flex flex-col justify-center items-center w-full max-w-[420px] mx-auto">
          ...
          <div className={`w-[360px] h-[360px] rounded-full border border-purple-500/15 border-dashed absolute bottom-[-180px] left-1/2 -translate-x-1/2 pointer-events-none transition-all duration-[2000ms] animate-platform-spin ${
            anim.platform ? "opacity-100 scale-100" : "opacity-0 scale-50"
          }`} />
          <div className={`w-[290px] h-[290px] rounded-full border border-cyan-500/20 border-dashed absolute bottom-[-145px] left-1/2 -translate-x-1/2 pointer-events-none transition-all duration-[2000ms] animate-platform-spin-reverse ${
            anim.platform ? "opacity-100 scale-100" : "opacity-0 scale-50"
          }`} />

          {/* Central Login Card (Rises with scale + opacity transition) */}
          <div className={`w-full p-[1.5px] rounded-3xl bg-gradient-to-tr from-purple-500/20 via-indigo-500/10 to-cyan-500/25 transition-all duration-[1500ms] ease-out shadow-2xl ${
            anim.centerCard ? "translate-y-0 opacity-100 scale-100" : "translate-y-28 opacity-0 scale-90 pointer-events-none"
          }`}>
            <div className="glass-futuristic-card p-6 md:p-8 rounded-3xl border border-white/5 relative overflow-hidden w-full shadow-purple-500/5 shadow-2xl">
              
              <div className="absolute inset-0 bg-[linear-gradient(to_bottom,transparent_50%,rgba(168,85,247,0.02)_50%)] bg-[size:100%_4px] pointer-events-none" />
              ...
              <div className="relative z-10 flex flex-col">
                ...
                <div className={`text-center mb-6 transition-all duration-[800ms] ease-out ${
                  ...
                  <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-tr from-purple-500/20 to-indigo-600/20 border border-purple-500/30 text-purple-400 shadow-lg shadow-purple-500/5 mb-3">
                    <GraduationCap className="w-7 h-7 animate-pulse" />
                  </div>
                  <h1 className="text-2xl font-extrabold text-white tracking-tight">
                    Student360 <span className="text-purple-400 text-neon-purple">AI</span>
                  </h1>
                  <p className="text-[10px] font-bold text-purple-400 uppercase tracking-widest mt-1 text-neon-purple">
                ...
                <form onSubmit={handleSubmit} className="space-y-4 font-sans" autoComplete="off">
                  ...
                  <div className={`transition-all duration-[800ms] ease-out ${
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2.5 bg-slate-950/60 border border-slate-800 hover:border-slate-700 focus:border-purple-500/80 rounded-xl text-xs text-white focus:outline-none focus:ring-2 focus:ring-purple-500/10 transition placeholder-slate-600 shadow-inner"
                  ...
                  <div className={`transition-all duration-[800ms] ease-out ${
                    ...
                    <PasswordInput
                      ...
                      inputClassName="bg-slate-950/60 border-slate-800 hover:border-slate-700 focus:border-purple-500/80 text-white focus:ring-2 focus:ring-purple-500/10 text-xs py-2.5"
                  ...
                  <div className={`flex items-center justify-between text-[10px] text-slate-400 transition-all duration-[800ms] ease-out ${
                    ...
                    <label className="flex items-center gap-1.5 cursor-pointer hover:text-white transition">
                      <input type="checkbox" className="accent-purple-500 rounded bg-slate-950 border-slate-800" />
                      <span>Remember session</span>
                    </label>
                    <button 
                      type="button" 
                      onClick={(e) => { e.preventDefault(); alert("For credentials reset, please contact the Department HOD or College Admin."); }}
                      className="hover:text-purple-400 transition hover:underline"
                    >
                      Forgot Password?
                    </button>
                  </div>
                  ...
                  <div className={`transition-all duration-[800ms] ease-out ${
                    ...
                    <button
                      ...
                      className="w-full py-2.5 px-4 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white text-xs font-semibold shadow-lg shadow-purple-500/10 border border-purple-400/20 hover:border-purple-400/30 transition flex items-center justify-center gap-2 group disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-purple-500/20 active:scale-[0.98] duration-150"
                    >
                      {loading ? (
                        <>
        ...
        <div className={`hidden lg:flex flex-col w-[260px] glass-futuristic p-5 rounded-2xl border border-cyan-500/15 shadow-lg shadow-cyan-950/20 transition-all duration-[1500ms] ease-out ${
          ...
          <div className="flex items-center gap-2 mb-3 text-cyan-400">
            <Cpu className="w-4 h-4 animate-pulse" />
            <span className="text-[11px] font-bold tracking-widest text-neon-cyan uppercase">Academic Context</span>
          </div>
          <div className="space-y-3 font-mono text-[10px] text-slate-400">
            <div className="p-2 bg-slate-950/50 rounded-lg border border-slate-800/80">
              <span className="text-slate-500">DEPT ACCREDITATION:</span>
              <div className="flex items-center gap-1.5 mt-0.5">
                <span className="text-white font-bold">AI & ML (CORE)</span>
              </div>
            </div>
            <div className="p-2 bg-slate-950/50 rounded-lg border border-slate-800/80">
              <span className="text-slate-500">ACADEMIC TIMELINE:</span>
              <span className="block text-white font-bold mt-0.5">2024 - 2028 COHORT</span>
            </div>
            <div className="p-2 bg-slate-950/50 rounded-lg border border-slate-800/80">
              <span className="text-slate-500">VAULT SECURITY:</span>
              <div className="flex items-center gap-1.5 mt-0.5">
                <span className="text-white font-semibold">SECURE TLS 1.3</span>
              </div>
            </div>
            <div className="p-2 bg-slate-950/50 rounded-lg border border-slate-800/80 flex items-center justify-between">
              <span className="text-slate-500">CERTIFICATE VAULT:</span>
              <span className="text-[9px] px-1.5 py-0.5 rounded bg-cyan-950/60 border border-cyan-800 text-cyan-300 font-bold uppercase">Active</span>
            </div>
          </div>
        </div>
